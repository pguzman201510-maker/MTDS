...
def _find_template() -> str:
    candidates = [
        ...
        "Z:\Deuda\EGDMP\EGDPMP 2026\MTDS_Bloomberg_Template.xlsx",
...
ORACLE_PATH = _find_data_file("Consulta_Oracle_31-03-2026.xls")  # TSV tab-separado con extensión .xls
PERFIL_PATH = _find_data_file("Perfil_de_vencimientos_2026.xlsm")
...
def load_from_oracle(oracle_path: str) -> dict:
    ...
    print(f"  │  FIJA     ({len(fijas):>2} tipos): {', '.join(fijas)}")
    print(f"  │  VARIABLE ({len(variables):>2} tipos):")
    for v in variables:
        desc = VARIABLE_TYPES_KNOWN.get(v, "—índice no mapeado—")
        print(f"  │     {v:<8} → {desc}")
    print("  │")
    print("  │  Regla: FIJA={FIJA,SINI,FUFI,FI19}")
    print("  │         MARGEN_VALOR=tasa nominal anual (fija) | spread (variable)")
    print("  └───────────────────────────────────────────────────────────────")

    # ── DEG / SDR → USD ──────────────────────────────────────────────────
    deg_count = (df["MDA_TR"] == "DEG").sum()
    if deg_count > 0:
        print(f"  ⓘ  DEG (SDR): {deg_count} préstamos reclasificados → USD")
    ...
    print(f"  │  {'Instrumento':<16} {'Moneda':>6} {'Tipo':>9}  "
    ...
    for (mda, tipo), grp in df.groupby(["MDA_TR_adj", "TIPO_CAT"]):
        ...
        print(f"  │  {inst_id:<16} {mda:>6} {tipo_lbl:>9}  "
              f"{saldo/1e6:>11.1f}M  {part:>6.2f}%  {tasa_lbl:>39}  {tpv:>5.2f}y")

    print(f"  │  {'-'*90}")
    print(f"  │  TOTAL EXTERNO: ${total_ext/1e6:.1f}M USD")
    print(f"  │  Monedas en portafolio: {', '.join(currencies)}")
    print(f"  └───────────────────────────────────────────────────────────────")
...
def load_from_perfil(perfil_path: str, oracle_result: dict = None) -> dict:
    ...
    print(f"  │  Stock total: {total_cop_mm/1e6:,.1f} billones COP")
    print(f"  │  {'Instrumento':<18} {'Stock COP Mm':>14} {'Part%':>7} "
          f"{'TPV':>6} {'%Vto Yr1':>10}")
    print(f"  │  {'-'*60}")
    rows_pv = [
    ...
    for name, stk, tpv_v, pct1 in rows_pv:
        ...
        print(f"  │  {name:<22} {stk:>14,.0f} {pct:>6.1f}% {tpv_v:>5.2f}y {pct1_str:>9}")

    # ── Primeros años del perfil ──────────────────────────────────────────
    print(f"  │  {'-'*60}")
    print(f"  │  Amortizaciones (COP Millones, primeros 8 años):")
    print(f"  │  {'Año':<6} {'TES COP':>12} {'TES UVR':>12} "
          f"{'Otros Int':>11} {'Ext Total':>12}")
    print(f"  │  {'-'*60}")
    for i, yr in enumerate(años[:8]):
        print(f"  │  {yr:<6} {tes_cop[i]:>12,.0f} {tes_uvr[i]:>12,.0f} "
...
def main():
    ...
    res_path = r"Z:\Deuda\EGDMP\EGDPMP 2026\MTDS_2026_Resultados.xlsx"
    save_results(w_opt, mc, stress, port, frontier, res_path)
    print(f"       ✔  {res_path}")

    print("\n[6/6]  Copiando script a outputs …")
    shutil.copy(__file__, r"Z:\Deuda\EGDMP\EGDPMP 2026\mtds_2026_v2_ejecutado.py")
    print("       ✔  \Deuda\EGDMP\EGDPMP 2026\mtds_2026_v2 (1).py")
